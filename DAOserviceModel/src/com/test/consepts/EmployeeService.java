...
public class EmployeeService implements EmployeeDao {
	...
	public void updateEmployee(Employee e) {
		try{
			Connection con=ConnectionFactory.getConnection();
			Statement stm=con.createStatement();
			int k=stm.executeUpdate("UPDATE `students1111`.`employee` SET `name` = '"+e.getName()+"',`address`='"+e.getAddress()+"',`pincode`='"+e.getPincode()+"' WHERE (`id` = '"+e.getEmpid()+"')");
	...
	public void deleteEmployee(Employee e) {
		...
		try{
			...
			int k=stm.executeUpdate("DELETE FROM `students1111`.`employee` WHERE (`id` = '"+e.getEmpid()+"')");
